...
**Docs-sync:** any phase that changes this file, `docs/taxonomy.md`, `docs/quiz-schema.md`, or `docs/person-schema.md` must check [`docs/writing-a-lesson.md`](writing-a-lesson.md) for staleness **in the same commit** — the contributor tutorial paraphrases these four contracts for teachability and rots if they drift.
...
Nodes are grouped by discipline one folder deep — `content/sociology/<slug>.md`, with a node's self-check quiz in a `quizzes/` folder beside it (`content/sociology/quizzes/<slug>.yml`). A second discipline lands as a sibling folder (`content/anthropology/…`). Two rules make this safe: **the directory carries no meaning for node identity — it is for humans browsing the repo**, and **the slug is the filename basename (without `.md`) and is globally unique across all of `content/`**, present and future. Every navigation graph, both completion stores, prerequisites, and cross-references key on that slug, so the loader finds a node by basename at any depth and the linter rejects a duplicate basename anywhere under `content/`.

One qualification, and it is a **clarification of long-standing behaviour, not a new exception**: a short allowlist of **non-node directory names** marks *sibling entity types*, which the node walkers skip and each type's own loader claims. Today that list is `quizzes` (see [`docs/quiz-schema.md`](quiz-schema.md)) and `people` (see [`docs/person-schema.md`](person-schema.md)). Quizzes have behaved this way since they shipped — they escaped the node walkers only because they are `.yml` — so the "no meaning to the platform" phrasing was already inaccurate before it was corrected here. The allowlist lives in `lib/content.ts` and `scripts/lint-content.mjs` as `NON_NODE_DIRS`. **Do not "restore" the original wording**: it describes node identity correctly and entity-type dispatch incorrectly, and a directory name is the only signal available for the latter. The slug rule is unaffected — people share the one global namespace with nodes.
