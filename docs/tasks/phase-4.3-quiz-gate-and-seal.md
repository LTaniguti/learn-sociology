...
**Unchanged doctrine:** completion remains the learner's deliberate act (the gate *unlocks* the button; it never auto-marks — Item 1), and the storage invariant holds absolutely: no new keys, no shape changes, the gate is **derived** from the two existing stores. **[Superseded in 4.4 by owner decision:** the "deliberate act / unlock, never auto-mark" half was overturned — for a published *choice*-quiz, completion is now **derived from mastery** (the lesson auto-marks on all-correct and the button is removed). The storage-invariant half stands unchanged (4.4 still adds no keys and writes through the existing `lib/progress` setter). See `phase-4.4-auto-complete-on-quiz.md`.**]**
...
- **Unlock, never auto-mark.** Reaching finished enables the button live (via `QUIZ_EVENT`); the learner still clicks. Rationale recorded: completion stays a deliberate act, no lesson silently completes mid-quiz, and unmarking stays coherent. **[Superseded in 4.4:** for a published choice-quiz this "unlock, never auto-mark" rule is reversed — finished now **auto-marks** the lesson (both hosts) and the button is removed. Reflect-only published quizzes are exempt and keep the manual button. See `phase-4.4-auto-complete-on-quiz.md`.**]**
