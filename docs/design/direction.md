...
7. **No accounts, ever.** Progress is device-local. Never imply login, profiles, streaks, or social. Metadata is limited to: title, summary, parent, prerequisites, related, tags, difficulty, people, source, status. Don't design anything that needs data outside that set (no ratings, view counts, authors, durations).
